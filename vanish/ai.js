...
(() => {
  ...
  function getAIParams() {
    return AI_CONFIGS[aiLevel] || AI_CONFIGS["medium"];
  }
  ...
  function findImmediateTactic(s, me, opp){
    ...
    for (let r=0; r<size; r++){
      for (let c=0; c<size; c++){
        ...
        if (isRenju && me === P1) {
          s[k] = P1;
          const res = window.violatesRenju(r, c, 1);
          s[k] = 0;
  ...
  function calculateRenjuTraps(s) {
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        ...
        // HUGE OPTIMIZATION: Only evaluate heavy foul logic if White has an intersecting threat
  ...
  function aiSwap2Choice() {
    ...
    if(bestPair) {
      if (window.setSwap2Phase) window.setSwap2Phase(3); // Fixes the transition lock
